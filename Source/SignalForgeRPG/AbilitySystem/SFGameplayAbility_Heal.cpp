	...
	FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	{
		UE_LOG(LogSFCombat, Warning, TEXT("[SF Heal] CanActivate=FALSE — Super rejected (cost/cooldown/blocked tags). Check Echo cost GE, Cooldown.EchoBased.Heal, and active tags."));
		return false;
	}
	...
	{
		UE_LOG(LogSFCombat, Warning, TEXT("[SF Heal] CanActivate=FALSE — AvatarActor is not an ASFCharacterBase."));
		return false;
	}

	if (!HealEffect)
	{
		UE_LOG(LogSFCombat, Warning, TEXT("[SF Heal] CanActivate=FALSE — HealEffect is unset on the GA. Assign a Heal GameplayEffect on GA_EchoHeal."));
		return false;
	}
	...
	{
		UE_LOG(LogSFCombat, Log, TEXT("[SF Heal] CanActivate=FALSE — Health is already at MaxHealth."));
		return false;
	}

	UE_LOG(LogSFCombat, Log, TEXT("[SF Heal] CanActivate=TRUE"));
	return true;
}
...
{
	UE_LOG(LogSFCombat, Warning, TEXT("[SF Heal] ActivateAbility ENTER"));

	ASFCharacterBase* Character = nullptr;
	if (!TryGetHealCharacter(ActorInfo, Character))
	{
		UE_LOG(LogSFCombat, Warning, TEXT("[SF Heal] Activate aborted — no character."));
	...
	{
		UE_LOG(LogSFCombat, Warning, TEXT("[SF Heal] Activate aborted — CommitAbility failed (cost GE could not be paid — not enough Echo, or cost GE is unset)."));
		EndAbility(Handle, ActorInfo, ActivationInfo, true, true);
		return;
	}
